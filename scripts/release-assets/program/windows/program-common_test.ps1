...
$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("agent-container-hub acl test {0}" -f [Guid]::NewGuid().ToString('N'))
...
try {
  ...
  [System.IO.File]::WriteAllText($envFile, "HUB_RUNTIME_DIR=C:\runtime`n")
  ...
  Write-Host '[test] agent-container-hub config ACLs remain readable'
} finally {
