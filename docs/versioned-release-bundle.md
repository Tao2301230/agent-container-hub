...
- `release-image`：固定打 `linux` 当前架构镜像
...
- 固定构建 Linux 镜像
- 默认使用当前机器架构
- 导出为 `docker save` 生成的镜像 tar，并额外 gzip 压缩
...
```bash
gzip -dc dist/release/agent-container-hub-image-v1.0.0-linux-arm64.tar.gz | docker load
```
...
- `make release-image` 产出 Linux 镜像 tar.gz
