...
func TestRenderWindowsManifest(t *testing.T) {
	...
	if err := render("../../scripts/release-assets/program/manifest.template.json", output, "v1.2.3", "windows", "amd64", "backend/agent-container-hub.exe", "agent-container-hub-v1.2.3-windows-amd64.zip"); err != nil {
		t.Fatal(err)
	}
